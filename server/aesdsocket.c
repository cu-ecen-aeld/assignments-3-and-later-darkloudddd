/**
 * aesdsocket – Assignment-5 native socket server (Step 7: graceful exit)
 *
 * Adds:
 *  • SIGINT/SIGTERM handler → sets global flag, logs “Caught signal, exiting”
 *  • accept() loop breaks when flag set
 *  • After loop: closes listening socket, unlinks data file, closelog()
 */
